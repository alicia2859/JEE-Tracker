...
export function loadHistoryData() {
    ...
    ensureDayShape(day);
    let sHtml = `<div style="margin-bottom:12px; padding:10px 14px; border:1px solid var(--border); border-radius:8px; display:inline-flex; gap:16px; flex-wrap:wrap;"><span><strong>Total Study:</strong> ${formatReadable(day.totalStudy)}</span><span style="color:var(--muted);">&amp;</span><span><strong>Total Break:</strong> <span style="color:#a78bfa;">${formatReadable(day.totalBreak)}</span></span></div>`;
...
export function deleteSubjectEntry(dt, subject) {
    ...
    if (!confirm(`Delete ${formatReadable(sec)} logged for ${subject} on ${dt}?`)) return;
...
export function deleteStudyLog() {
    ...
    if (!confirm(`Delete all study logs for ${dt}?`)) return;
...
export function deleteBreakLog() {
    let dt = document.getElementById("history-picker").value; if (!dt) return;
    let db = getDB(); if (!db[dt]) { alert("No data."); return; }
    if (!confirm(`Delete all break logs for ${dt}?`)) return;
